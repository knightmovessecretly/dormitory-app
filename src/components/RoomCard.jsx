  ...
  return (
    <>
      ...
      >
        {/* IMAGE */}
        <div
          className="relative mt-6 mx-4 h-[260px] flex items-center justify-center bg-gray-100 rounded-2xl overflow-hidden"
          {...handlers}
        ...
        {room.images.length > 1 && (
          <div
            className="flex justify-center gap-2 p-3 overflow-x-auto"
            onClick={(e) => e.stopPropagation()}
          >
            {room.images.map((image, index) => (
              <img
                ...
                className={`w-14 h-12 rounded-lg object-cover cursor-pointer transition ${
                  currentImage === index
...
<div className="flex-1 flex flex-col px-6 pb-6 text-center">
  {/* Room Name */}
  <h3 className="text-4xl font-extrabold text-[#FF00FF] leading-tight mt-4">
    {room.name}
  </h3>

  {/* Room Description */}
  {room.description && (
    <p className="mt-4 text-gray-600 text-sm leading-relaxed max-w-sm mx-auto">
      {room.description}
    </p>
  )}
